# formatting-spec — Padrão CNTR (cabeçalho/rodapé + numeração) — radec-contabilidade

Especificação de formatação das **PROPOSTAS e CONTRATOS** contábeis RadéC. Mesmo
layout CNTR do padrão geral (`rs-advogados`), trocando apenas a cor padrão da
identidade RadéC no título e no fio do cabeçalho. Gerado com a biblioteca
[`docx`](https://docx.js.org/).

- Página 1 sem cabeçalho.
- Página 2+: topo com `Título | CNTRNNNNNN`, fio na cor RadéC.
- Rodapé (todas as páginas): número à esquerda, `Página X de Y` à direita, **sem**
...
// Identidade visual RadéC
const RADEC_VERDE = "2C5F5F"; // verde petróleo (título / fio do cabeçalho)
const RADEC_AZUL  = "1F4E79"; // azul (alternativa de destaque)
const COR_TITULO  = RADEC_VERDE;
const COR_FIO     = RADEC_VERDE;
```
...
Página 1 sem cabeçalho. A partir da página 2: `Título | CNTRNNNNNN`, com o fio na
cor RadéC.
...
`Página X de Y` à direita. Sem dados institucionais.
...
## 4. Ligar tudo na seção do Document
